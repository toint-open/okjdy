...
import cn.toint.jdy4j.core.model.JdyFieldListResponse;
...
public class JdyWidgetHolder {
    private static final ThreadLocal<JdyFieldListResponse> THREAD_LOCAL = new TransmittableThreadLocal<>();

    public static JdyFieldListResponse get() {
        return THREAD_LOCAL.get();
    }

    public static JdyFieldListResponse getRequire() {
        final JdyFieldListResponse result = THREAD_LOCAL.get();
        Assert.notNull(result, "当前上下文环境信息不存在");
        return result;
    }

    public static void set(JdyFieldListResponse widgetResponse) {
